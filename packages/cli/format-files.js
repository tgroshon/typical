...
const logger = getLogger({ prefix: "typical-cli" });

const messages = {
  success(data) {
    const files = `{count, plural, one{file} other{files}}`;
    return mf.compile(
      `{success} formatting {countString} ${files} with typical`
    )(data);
  },
  ...
  failure(data) {
    const files = `{count, plural, one{file} other{files}}`;
    return mf.compile(
      `{failure} formatting {countString} ${files} with typical`
    )(data);
  },
...
export default function formatFilesFromArgv({
  _: fileGlobs,
